...
% Apply Gaussian smoothing to ensure smooth transitions
Z = imgaussfilt(Z, 1);
...
for i = 1:length(path_x)
    ...
    % Update collected data visualization
    if ~isempty(collected_points_handle)
        delete(collected_points_handle);
    end
    collected_x = path_x(1:i);
    collected_y = path_y(1:i);
    collected_z = path_z(1:i);
    collected_points_handle = scatter3(collected_x, collected_y, collected_z, "o", "filled", 'g');
