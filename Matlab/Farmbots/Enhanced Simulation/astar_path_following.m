...
% Set improved lighting and colormap
lighting gouraud;  % Smoother lighting than phong[7]
camlight('headlight');
camlight(45, 45);  % Add second light source for better illumination[6]
shading interp;

% Use a brighter colormap and enhance it
colormap(parula);
brighten(0.6);  % Brighten the colormap by 60%[2]
...
% Create initial rover patch with stable color properties
roverPatch = patch('Vertices', vertices, 'Faces', faces, 'FaceColor', [1 0 0], 'EdgeColor', 'black', 'FaceAlpha', 0.9,'FaceLighting', 'none', 'AmbientStrength', 1.0,'DiffuseStrength', 0.0);
...
% Set start position in grid coordinates
start_pos = [1, 1];

% Get optimal leveling path using A*
[path, visit_count] = terrain_leveling_astar(avg_height_grid, start_pos);

% Convert path indices to actual coordinates with proper scaling
path_x = (path(:,2) - 0.5) * cell_size;
path_y = (path(:,1) - 0.5) * cell_size;

% Ensure path coordinates are within terrain bounds
smooth_path_x = max(0, min(5, path_x));
smooth_path_y = max(0, min(5, path_y));

% Get Z coordinates through interpolation
smooth_path_z = interp2(X, Y, Z, path_x, path_y);
...
video = VideoWriter('rover_simulation.mp4', 'MPEG-4'); % Create video object
video.FrameRate = 30; % Set frames per second
open(video);
...
for i = 1:length(smooth_path_x)
    ...
    % Force consistent rover color in every frame
    set(roverPatch, 'FaceColor', [1 0 0]);
                ...
                if local_ix > 0 && local_ix <= size(Z, 2) && local_iy > 0 && local_iy <= size(Z, 1)
                    ...
                    if current_passes < required_passes
                        ...
                        Z(local_iy, local_ix) = Z(local_iy, local_ix) - ...
                            (Z(local_iy, local_ix) - 0) * reduction_factor;
                    end
                end
            end
        end
